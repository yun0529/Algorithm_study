//#include <bits/stdc++.h>
//using namespace std;
//
//int r = 0, c = 0, flag = 0, check = 1;
//char arr[1004][1004];
//int visited[1004][1004];
//
//int dy[] = { -1,0,1,0 };
//int dx[] = { 0,1,0,-1 };
//
//queue<pair<int, int>> q;
//vector<pair<int, int>> fireLocation, removeFire;
//pair<int,int> ji;
////list<pair<int, int>>::iterator iter;
//
//void fire(int y, int x) {
//	//cout << y << " : " << x << '\n';
//	for (int i = 0; i < 4; i++) {
//		int ny = y + dy[i];
//		int nx = x + dx[i];
//
//		if (ny < 0 || nx < 0 || ny >= r || nx >= c || arr[ny][nx] == 'F' || arr[ny][nx] == '#') continue;
//		arr[ny][nx] = 'F';
//		fireLocation.push_back({ ny, nx });
//		//cout << ny << " : " << nx<<'\n';
//	}
//	//if(fireLocation.size()) fireLocation.erase(fireLocation.begin());
//	removeFire.push_back({ y, x });
//	return;
//}
//
//void bfs(int y, int x) {
//	visited[y][x] = 1;
//	q.push({ y,x });
//	while (q.size()) {
//		tie(y, x) = q.front();
//		q.pop();
//		int size = fireLocation.size();
//		//cout << "size : " << size << '\n';
//
//		if (check <= visited[y][x]) {
//			//for (iter = fireLocation.begin(); iter != fireLocation.end(); iter++) {
//			//	fire(iter->first, iter->second);
//			//}
//			for (int i = 0; i < size; i++) {
//				fire(fireLocation[i].first, fireLocation[i].second);
//				//check = 0;
//				//arr[v.first][v.second] = 'F';
//			}
//			check++;
//		}
//		for (int i = 0; i < 4; i++) {
//			int ny = y + dy[i];
//			int nx = x + dx[i];
//
//			if (ny < 0 || nx < 0 || ny >= r || nx >= c) continue;
//			if (visited[ny][nx] || arr[ny][nx] == 'F' || arr[ny][nx] == '#') continue;
//			visited[ny][nx] = visited[y][x] + 1;
//			if (ny == 0 || nx == 0 || ny == (r - 1) || nx == (c - 1)) {
//				//cout << ny << " : " << nx << '\n';
//				cout << visited[ny][nx]<<'\n';
//				flag = 1;
//				return;
//			}
//			q.push({ ny,nx });
//			
//		}
//		//for (int i = 0; i < r; i++) {
//		//	for (int j = 0; j < c; j++) {
//		//		cout << visited[i][j];
//		//	}
//		//	cout << "\n";
//		//}
//		//cout << "\n";
//		//for (int i = 0; i < r; i++) {
//		//	for (int j = 0; j < c; j++) {
//		//		cout << arr[i][j];
//		//	}
//		//	cout << "\n";
//		//}
//		//cout << "=====================\n";
//	}
//	return;
//}
//
//
//int main() {
//
//	ios_base::sync_with_stdio(false);
//	cin.tie(NULL); cout.tie(NULL);
//
//	cin >> r >> c;
//
//	for (int i = 0; i < r; i++) {
//		for (int j = 0; j < c; j++) {
//			cin >> arr[i][j];
//			if (arr[i][j] == 'J') {
//				ji = { i,j };
//			}
//			else if (arr[i][j] == 'F') fireLocation.push_back({ i,j });
//		}
//	}
//	if (ji.first == 0 || ji.second == 0) {
//		cout << 1;
//		return 0;
//	}
//	bfs(ji.first, ji.second);
//	//for (int i = 0; i < r; i++) {
//	//	for (int j = 0; j < c; j++) {
//	//		cout << visited[i][j];
//	//	}
//	//	cout << "\n";
//	//}
//	if (!flag) cout << "IMPOSSIBLE";
//	
//	return 0;
//}
//
////#include <bits/stdc++.h> 
////using namespace std;
////const int INF = 987654321;
////char a[1004][1004];
////int n, m, sx, sy, dx[4] = { -1,0,1,0 }, dy[4] = { 0,-1,0,1 }, ret, y, x;
////int fire_check[1004][1004], person_check[1004][1004];
////bool in(int a, int b) {
////	return 0 <= a && a < n && 0 <= b && b < m;
////}
////int main() {
////	ios_base::sync_with_stdio(false);
////	cin.tie(0); cout.tie(0);
////	queue<pair<int, int>> q;
////	cin >> n >> m;
////	fill(&fire_check[0][0], &fire_check[0][0] + 1004 * 1004, INF);
////	for (int i = 0; i < n; i++) {
////		for (int j = 0; j < m; j++) {
////			cin >> a[i][j];
////			if (a[i][j] == 'F') {
////				fire_check[i][j] = 1; q.push({ i, j });
////			}
////			else if (a[i][j] == 'J') {
////				sy = i; sx = j;
////			}
////		}
////	}
////
////	while (q.size()) {
////		tie(y, x) = q.front();
////		q.pop();
////		for (int i = 0; i < 4; i++) {
////			int ny = y + dy[i];
////			int nx = x + dx[i];
////			if (!in(ny, nx)) continue;
////			if (fire_check[ny][nx] != INF || a[ny][nx] == '#') continue;
////			fire_check[ny][nx] = fire_check[y][x] + 1;
////			q.push({ ny, nx });
////		}
////	}
////
////	person_check[sy][sx] = 1;
////	q.push({ sy,sx });
////	while (q.size()) {
////		int y = q.front().first;
////		int x = q.front().second;
////		q.pop();
////		if (x == m - 1 || y == n - 1 || x == 0 || y == 0) {
////			ret = person_check[y][x];
////			break;
////		}
////		for (int i = 0; i < 4; i++) {
////			int ny = y + dy[i];
////			int nx = x + dx[i];
////			if (!in(ny, nx)) continue;
////			if (person_check[ny][nx] || a[ny][nx] == '#') continue;
////			if (fire_check[ny][nx] <= person_check[y][x] + 1) continue;
////			person_check[ny][nx] = person_check[y][x] + 1;
////			q.push({ ny, nx });
////		}
////	}
////	if (ret != 0) cout << ret << "\n";
////	else cout << "IMPOSSIBLE \n";
////	return 0;
////}